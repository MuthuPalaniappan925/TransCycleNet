...
def define_discriminator(image_shape):
	...
	in_image = InputLayer(input_shape=image_shape)
	# Conv64: [4x4 kernel] [Stride 2x2]
	d = Conv2D(64, (4,4), strides=(2,2), padding='same', kernel_initializer=init)(in_image)
	d = LeakyReLU(alpha=0.2)(d)
	# Conv128: [4x4 kernel] [Stride 2x2]
	d = Conv2D(128, (4,4), strides=(2,2), padding='same', kernel_initializer=init)(d)
	d = InstanceNormalization(axis=-1)(d)
	d = LeakyReLU(alpha=0.2)(d)
	# Conv256: [4x4 kernel] [Stride 2x2]
...
def define_composite_model(g_model_1, d_model, g_model_2, image_shape):
	...
	# adversarial loss
	input_gen = InputLayer(input_shape=image_shape)
	gen1_out = g_model_1(input_gen)
	output_d = d_model(gen1_out)
	# identity loss
	input_id = InputLayer(input_shape=image_shape)
